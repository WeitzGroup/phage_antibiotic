%% Code for the robustness analysis of the partial resistance model: Antibiotic + Innate immunity therapeutic
...
% Simulate the antibiotic therapeutic + immune response model for varied
...
colormap(cmap);
h = colorbar;
%set(h, 'location', 'manual', 'position', [0.8482 0.10 0.0286 0.8155])
...
yt = get(gca, 'YTick');
for j=1:length(yt)
  tx=text(-.5 , yt(j), ['10^{' num2str(log10( concentrations(yt(j)))) '}']);
  set(tx,'HorizontalAlignment','center','fontsize',get(gca,'fontsize'), 'fontweight', 'bold')
end
set(gca, 'fontweight', 'bold')
title({"Antibiotic + Immune response"; "MIC_{CP} for B_{P} = 0.172 \mug/ml, MIC_{CP} for B_{A} = 0.014 \mug/ml"}, 'FontSize', 20)
