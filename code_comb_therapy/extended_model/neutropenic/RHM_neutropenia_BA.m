% Code to simulate the partial resistance-heterogeneous mixing model (rhmODE)
% Neutropenic host
...
dose = 0.014*2.5; % ug/ml, 2.5 x MIC of ciprofloxacin
anti_name = 'CP';
...
% Simulate partial resistance model-combination therapy in a neutropenic
% host given an antibiotic-sensitive inoculum
[y, TB, time] = simRHM(Ki, Io, 0, B, P, dose, anti_name, delta_P);
...
if P ~= 0
    ...
    v = [g(1) h(1) j(1)]; 
    h_leg = legend(v, 'phages','BP','BA', 'Location','northeast');
else
    % Legend for condition with phage treatment
    v = [ h(1) j(1)];
...
title({"Phage-Antibiotic combination against a B_{a} inoculum"; "MIC_{CP} for B_{P}= 0.172 \mug/ml, MIC_{CP} for B_{A} = 0.014 \mug/ml"}, 'FontSize', 20)