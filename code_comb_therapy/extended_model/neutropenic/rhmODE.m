...
kkill = p.kkill;
% EC_50 of the antibiotic-sensitive bacteria, antibiotic concentration when kkill/2
ec = p.ec;
...
% Extended model (phage-sensitive bacteria are semi-sensitive to
% antibiotic, antibiotic-sensitive bacteria are semi-sensitive to phages)

% EC50 of the phage-sensitive strain, calculated using the MIC value of the
% phage-sensitive strain
ec_bp= p.ec_bp;

% delta_P, modulates the level of phage-resistance of the
...
dx(1) = (r*B*(1-((B+R)/Kc))*(1-m))-(phi*B*(P^g))-(ep*I*B/(1+((B+R)/Kd)))+ rp*R*(1-((B+R)/Kc))*m2 - ((kkill*(A^H)*B)/(ec_bp^H + A^H)); % with reversible mutation
% Change in resistant bacterial population
%dx(2) = ((rp*R*(1-((B+R)/Kc)))+r*B*(1-((B+R)/Kc))*m)-(ep*I*R/(1+((B+R)/Kd))) -((kkill*(A^H)*R)/(ec^H + A^H));
dx(2) = ((rp*R*(1-((B+R)/Kc)))*(1-m2)+r*B*(1-((B+R)/Kc))*m)-(ep*I*R/(1+((B+R)/Kd))) - ((kkill*(A^H)*R)/(ec^H + A^H)) - (delta_P*phi*R*(P^g)); % with reversible mutation
% Change in phage population
dx(3) = (beta*phi*B*(P^g))+ (delta_P*beta*phi*R*(P^g)) - (w*P);
