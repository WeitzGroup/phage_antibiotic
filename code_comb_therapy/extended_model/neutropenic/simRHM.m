...
%         (3)phage-sensitive bacteria- initial bacteria conc.,
%         (4)antibiotic-sensitive bacteria- initial bacteria conc.
%         (5) phage- initial phage conc.
%         (6) antibiotic - initial injected dose, (7) Antibiotic name
%         (8) delta_P - modulates the level of phage resistance of the
...
function [ y, TB, time ] = simRHM( Ki, immune, Bp, Ba, phage, varargin)
   ...
   %---------parameters--------------
    ...
    % Delta, modulates the phage adsorption rate (i.e., phage infection) of
    % antibiotic-sensitive bacteria
    p.delta_P = delta_P;
    ...
    if strcmp('CP', anti_name) % Ciprofloxacin parameters
        ...
        p.kkill = 18.5;
        % EC_50 of the antibiotic-sensitive strain (antibiotic
        % concentration when kkill/2)
        p.ec = 0.3697;
        ...
        % EC_50 of phage-sensitive bacterial strain
        p.ec_bp = 4.0707;
    ...
    check = 0;
    currentTime = t2(end);
    if currentTime < 1000 % Bacterial pop died before end of simulation
        check = 1;
        B2 = y2(end,1);
        if B2 <= 1
            B2 = 0;
