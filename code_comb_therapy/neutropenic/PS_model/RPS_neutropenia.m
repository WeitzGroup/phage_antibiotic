...
dose = 0.014*2.5; % 2.5 x MIC of ciprofloxacin for BA strain
anti_name = 'CP';
...
[y, TB, time] = simRPS(Ki, Io, B, 0, P, dose, anti_name);
...
if P ~= 0
    ...
    v = [g(1) h(1) j(1)];
    h_leg = legend(v, 'phage','BP','BA', 'Location','southeast');
else
    % Legend for condition with phage treatment
    v = [ h(1) j(1)];
    h_leg = legend(v, 'BP','BA', 'Location','southeast');
end
...
title({"Phages + Antibiotic against a B_{P} inoculum";"Phage saturation model"}, 'FontSize', 20)
