%Function to simulate lirODE for immunodeficient hosts
% inputs: (1)Ki- total immune capacity, (2)immune- initial immune response, 
%         (3)phage-sensitive bacteria- initial bacteria conc., (4) antibiotic-sensitive bacteria- initial bacteria con
...
function [ y, TB, time ] = simLIR( Ki, immune, Bp, Ba, phage, varargin )
   ...
   %---------parameters--------------
    ...
    [t1,y1] = ode45(@lirODE,tspan,y0,options,p);
    %----------------------------------------
    ...
    [t2,y2] = ode45(@lirODE,tspan2,yi,options,p);
    ...
    if currentTime < 1000 % Bacterial pop died before end of simulation
        ...
        % simulating diff eq
        [t3,y3] = ode45(@lirODE,tspan3,yii,options,p);
    end
    %----------------------------------------
    % continue simulation after susceptibles or resistants die
    ...
    if currentTime2 <= 999
        ...
        % simulating diff eq
        [t4,y4] = ode45(@lirODE,tspan4,yiii,options,p);
