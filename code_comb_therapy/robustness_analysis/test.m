...
for start = 1:length(dosing_time)
    ...
    %anti_conc = [anti_conc 0];
    %anti_conc = sort(anti_conc, 'descend');
    ...
    %ylabel('Fold Change in MIC','FontSize',20, 'Position', [-1.5 50])
    ...
    for j=1:length(yt)
      ...
      %h=text(-.5 , yt(j), num2str(concentrations(yt(j))));
      set(h,'HorizontalAlignment','center','fontsize',get(gca,'fontsize'), 'fontweight','bold')
    end
    set(gca, 'fontweight', 'bold')
    %title({'A + P + I', ['Treatment ' int2str(dosing_time(start)) ' h after infection']}, 'FontSize', 20)
...
%     hold on
%     plot([0 22], [31.5 31.5], '-r', 'Linewidth', 3 )
%     text(15 , 29, '10^{-0.5}','fontsize', 16,'fontweight', 'bold' );
%     plot([0 22], [11.5 11.5], '-r', 'Linewidth', 3 )
%     text(15 , 10, '10^{0.5}','fontsize', 16,'fontweight', 'bold' );
%     hold off
